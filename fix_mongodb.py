"""
Script pour corriger la base MongoDB et ajouter les produits.
"""
...
def fix_mongodb():
    """Corrige la base MongoDB et ajoute les produits."""
    try:
        ...
        # Vérifier la collection product
        product_count = db.product.count_documents({})
        print(f"Produits dans 'product': {product_count}")
        
        # Vérifier la collection products
        products_count = db.products.count_documents({})
        print(f"Produits dans 'products': {products_count}")
        
        if product_count == 0 and products_count == 0:
            print("Ajout des produits dans la collection 'product'...")
            ...
            print(f"Ajout de {len(products_data)} produits dans la collection 'product'")
            
            # Vérifier l'ajout
            final_count = db.product.count_documents({})
            print(f"Nombre total de produits dans 'product': {final_count}")
        else:
            print("Des produits existent deja dans la base de donnees")
        
        client.close()
        print("Base de donnees MongoDB corrigee avec succes!")
